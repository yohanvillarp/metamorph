...
const integrationProcessor = {
  async apply({ event, participant }: SituationContext) {
    ...
    if (locks.has(planId)) {
      console.log(
        `[IntegrationAgent] Integration already running for ${planId}. Ignoring duplicate phase event.`,
      );
      return;
    }
    locks.add(planId);
    ...
    try {
      ...
      if (plan.phase === "completed" || plan.phase === "failed") {
        console.log(
          `[IntegrationAgent] Plan ${planId} already finished (${plan.phase}). Ignoring extra integration phase.`,
        );
        return;
      }
      ...
      const {
        ...
        createRunBuildTool,
        runInShadowWorkspace,
        parseImplicatedFiles,
      } = await import("@nikelyh/infrastructure");
      const { collectShadowIssues, collectRepairTargets } =
        await import("../migration/registry");
      const { resolveMigrationCatalog } = await import("@nikelyh/domain");
      const shadowPath =
      ...
      log(
        producerId,
        planId,
        `File migration is done. Next: install dependencies in the shadow workspace (${pmCommands.cleanInstall}). This can take several minutes — counters staying at 0 is expected.`,
      );
        ...
        (chunk) => {
          ...
          if (Date.now() - lastInstallBeat >= 15000) {
            ...
            log(
              producerId,
              planId,
              `${pmCommands.cleanInstall} is still running in the shadow workspace. Wait — this is not finished.`,
            );
          }
        },
      );
      if (!install.ok) {
        ...
        log(
          producerId,
          planId,
          `${pmCommands.cleanInstall} failed in the shadow workspace. Halting build to avoid noisy compile errors.`,
          "error",
        );
        ...
        if (lastRound) {
          await failMigration(
            producerId,
            planId,
            `${pmCommands.cleanInstall} failed in the shadow workspace after ${MAX_INTEGRATION_ROUNDS} integration rounds. The migration is not successful.`,
          );
          return;
        }
      ...
      log(
        producerId,
        planId,
        `${pmCommands.cleanInstall} finished. Next: ${pmCommands.runBuild} in the shadow workspace.`,
      );
      ...
      if (!build.ok) {
        ...
        const { createAgent: createMozaikAgent } =
        ...
        await new Promise<void>((originalResolve) => {
          ...
          const tempAgent = createMozaikAgent({
            ...
            instruction:
              "You are a staff engineer repairing a migrated project. Read failing files and their imports. Deduce real APIs from disk. Do not invent prop names. Do not wait for Git.",
            tools: shadowTools,
            handlers: [],
          });
...
When you have applied fixes (or cannot fix further), stop. A deterministic rebuild will run after you finish.`;
          ...
          setTimeout(() => {
            if (!isDone) {
              console.warn(
                "[IntegrationAgent] Repair loop timed out; continuing with deterministic rebuild.",
              );
              leave(tempAgent);
              resolve();
            }
      ...
      const catalogEntry = resolveMigrationCatalog(
        plan.profile.source,
        plan.profile.target,
      );
      const structureIssues = collectShadowIssues(
        shadowPath,
        pluginCtx,
        catalogEntry?.layer,
      );
      ...
      if (!lastRound && repairFiles.length > 0) {
        log(
          ...
          planId,
          buildFailed
            ? `${pmCommands.runBuild} (or ${pmCommands.cleanInstall}) failed. Assigning repair tasks to Worker — the run is not finished.`
        ...
        await requeueBrokenFiles(
          ...
          planId,
          repairFiles,
          errors.length > 0 ? errors : ["Shadow verification failed."],
        );
        return;
      }
      ...
      await failMigration(
        ...
        planId,
        lastRound
          ? `${pmCommands.runBuild} still does not pass after ${MAX_INTEGRATION_ROUNDS} integration rounds. The migration is not successful.`
      ...
      await failMigration(
        producerId,
        planId,
        "Integration Phase encountered an error. The migration is not successful.",
      );
    } finally {
...
export function createIntegrationAgent(tools: Tool[] = []): Agent {
  return createAgent({
    ...
    capabilities: ["integration_testing", "code_repair"],
    instruction:
      "You are the Integration Agent. You run npm install and npm run build in the shadow workspace. A passing compile is not enough if catalog verifiers fail. Never mark the migration complete unless the shadow build passes and verifiers are clean. On failure, reopen file tasks for Worker.",
    tools: tools,
